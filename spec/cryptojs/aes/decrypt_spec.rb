RSpec.describe Cryptojs::Aes::Decrypt do
  it 'has a version number' do
    expect(Cryptojs::Aes::Decrypt::VERSION).not_to be nil
  end

  it 'decrypts-ciphers' do
    cipher = ''
    passphrase = 'passcode'
    expected = '{"id":"0a09a9e1-eee2-4a8e-bf77-901be943621c","key":"WHAT","value":"IS_THIS_NAH","comment":"","addedBy":{"name":"Aleem Isiaka","id":"a3748a40-3ab8-4dd1-bac6-e73c1047c95f"},"addedDate":"April 22, 2022 | 20:09 GMT","method":""}'
    expect(Cryptojs::Aes::Decrypt.run(cipher, passphrase)).to eq(expected)
  end
end
